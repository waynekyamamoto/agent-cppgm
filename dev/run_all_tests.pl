...
for my $test (@tests)
{
	...
	if ($assignment eq "pa1") {
		print $bf "$test_out.exit_status str $test_out $test_out $test ./$app\n";
	} elsif ($assignment eq "pa2" || $assignment eq "pa3" || $assignment eq "pa4") {
		print $bf "$test_out.exit_status str $test_out $test_out.stderr $test ./$app\n";
	} elsif ($assignment eq "pa5" || $assignment eq "pa6" || $assignment eq "pa7") {
		chomp(my @inputs = `ls $test*`);
